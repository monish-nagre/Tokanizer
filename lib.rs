...
declare_id!("A2tkVKmiwS8XaD4HyczAsPcunD7Ce2V4mtCKbfPN1w6o");
...
pub mod anchor_auction {
    ...
    pub fn exhibit(
        ...
        AuthorityType::AccountOwner,                   // set_authority to => exhibitor_nft_temp_account
        Some(pda)
        )?;

        token::transfer(
            ctx.accounts.to_transfer_to_pda_context(),   // transfer =>  from: exhibitor_nft_token_account
           1                                             //               to : exhibitor_nft_temp_account
        )?;
    ...
    pub fn buynft(ctx: Context<Buynft>, buynft: u64) -> Result<()> {
        let (pda, bump_seed) = Pubkey::find_program_address(&[ESCROW_PDA_SEED], ctx.program_id);
        let _signers_seeds: &[&[&[u8]]] = &[&[&ESCROW_PDA_SEED[..], &[bump_seed]]];
...
pub struct Buynft<'info> {
    ...
    #[account(
        ...
        constraint = escrow_account.highest_bidder_ft_temp_pubkey == highest_bidder_ft_temp_account.key(),
        constraint = escrow_account.highest_bidder_ft_returning_pubkey == highest_bidder_ft_returning_account.key(),
        constraint = escrow_account.sell_price == price,
        constraint = escrow_account.end_at > clock.unix_timestamp
    )]
    pub escrow_account: Box<Account<'info, Auction>>,
    pub clock: Sysvar<'info, Clock>,
